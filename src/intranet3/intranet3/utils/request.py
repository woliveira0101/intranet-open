...
class Request(request.Request):
    ...
    def user(self):
        ...
        if user_id:
            return DBSession.query(models.User).get(user_id)
    ...
    @reify
    def wants_mobile(self):
        if not 'wants_mobile' in self.session:
            self.session['wants_mobile'] = self.is_mobile
        elif 'wants_mobile' in self.params:
            self.session['wants_mobile'] = self.params['wants_mobile'] == '1'
        return self.session['wants_mobile']
