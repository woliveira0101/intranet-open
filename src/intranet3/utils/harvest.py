...
class Harvest(object):
    ...
    def _request(self, type = "GET", url = "", data = None):
        if type != "DELETE":
            if data:
                r = requests.post(url, data=data, headers=self.headers, auth=HTTPBasicAuth(self.email, self.password), verify=False)
            else:
                if not url.endswith(".json"): #dont put headers it a status request
                    r = requests.get(url=url, headers=self.headers, auth=HTTPBasicAuth(self.email, self.password), verify=False)
                else:
            ...
            try:
                r = requests.delete(url, headers=self.headers, auth=HTTPBasicAuth(self.email, self.password), verify=False)
            except Exception as e:
                raise HarvestError(e)
